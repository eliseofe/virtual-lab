...
The current bounded **#273 — UI/UX refinement and visual polish from real use** pass is **#392 — Experiment revision time and actor provenance**. The Experiment library exposes the exact timestamp and actor of the currently stored revision using metadata already present on the Experiment row; no revision/provenance storage model changes are introduced. #273 remains a living evidence-driven domain and returns dormant after this bounded pass.
...
1. **#392 — Experiment revision time and actor provenance is the current bounded UI/UX pass.** It exposes existing current-revision timestamp and actor metadata directly in the Experiment library without changing storage semantics.
...
3. **#273 UI/UX** returns dormant/living when #392 reaches its terminal state.
