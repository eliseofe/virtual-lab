...
The bounded **#392 — Experiment revision time and actor provenance** and **#394 — Human identity** passes are complete. The owner-authorized **#396 — Experiment working copy and navigable revision history** mini-epic is now active under living UI/UX lane #273. Its first child **#397** is production-complete: numbered Experiment revisions are retained immutable snapshots, human edits autosave into one unnumbered owner Working copy at interaction boundaries, and only explicit **Save Revision** crystallizes that Working copy. **#398** is now the active child: revision navigation moves into the Experiment header, retained snapshots and the Working copy are directly selectable, provenance/time/base lineage are visible, All/Mine/AI filtering is available, and Realtime head signals surface newly arrived AI revisions without switching away from the current view. Existing Working-copy edits remain protected while numbered revisions are inspected.
...
1. **#396 — Working copy and revision history is the active bounded UI/UX mini-epic.** #397 is production-complete. #398 is the current substantial ticket: it adds the holistic Experiment-header revision selector/history UI, exact snapshot navigation, All/Mine/AI filtering, and immediate visibility of newer AI revisions while preserving the Working copy. Its known successor #399 remains not started.
