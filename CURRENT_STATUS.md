...
- **#425 Refactoring / technical debt** — living domain, active again from 24 September under the owner-approved plan in `docs/REFACTORING_AUDIT_2026-09-24.md`, executed one ticket at a time by Claude: #532 behaviour-preservation safety net, #533 reliable release loop, #535 proposal-first merging, #537 simulator core tidy-up, #541 shared database connection and #543 styles out of JavaScript, #545 revision behaviour tests, #547 Library behaviour tests, #551 Professor inbox behaviour tests and #552 Showcase curation behaviour tests (production-verified), then #554 splitting the Experiment registry module into five tested parts (production-verified), and the owner-approved four-stage model-view-controller restoration: #560 runtime model (stage 1) and #562 simulation controller (stage 2), both production-verified; #564 results and authoring panels (stage 3, production-verified), #567 one authoring controller with a direct Apply command (production-verified), then #569 retiring the duplicate legacy views and whole-page watchers (stage 4, completed: whole-page watchers skip live runtime updates, hidden legacy simulation labels are redrawn only while visible, other modules drive the simulator and results through controller commands instead of pressing hidden buttons, and page events use one `vlab:` prefix documented in `docs/ARCHITECTURE.md` §19). The four-stage model-view-controller plan is complete; the three page-wide discovery observers remain by design, filtered of live runtime traffic. **Next under #425, owner-approved on 25 September:** the authoring-language design (`docs/DECISIONS.md` D-021 one code grammar with per-artifact views, and D-022 anonymous robots; rationale in `docs/REFACTORING_AUDIT_2026-09-24.md` §9), executed as #575 MCP live commit check (done), #576 shared translator core (done: `web/src/authoring-core/` tokenizer, expression and statement parsers and line helpers; behaviour pinned by `web/tests/authoring-equivalence.test.mjs`), then #577 grammar unification (contract change, in progress in five releases: 1 operators everywhere, done, `vlab.authoring/0.10`; 2 bounded `range` loops in Controller and Metrics, done, `vlab.authoring/0.11`; 3 Initialization on the shared statement parser with static typing, done, `vlab.authoring/0.12`; 4 roles replacing `set_agent_state` and robots hidden from N/ARENA_SIZE/EXPERIMENT_DURATION, done, `vlab.authoring/0.13` (Sergio Gutierrez's student Experiment 'Swarm vs Swarm - Case 1' still uses set_agent_state; a migrated source is prepared for its owner; follow-up fix: sources that fail to compile at startup, such as a restored pre-roles Initialization, no longer lock applying and Library opening, checked by `web/scripts/startup-recovery-smoke.mjs`); 4b one heterogeneity mechanism (owner decision D-023, 26 September): groups declare who differs (`group`, independent partitions), and `set_state`/`equip` attach starting memory and reference sensors to groups, replacing `role(...)` and the per-robot `set_agent_reference_sensor(...)`, `vlab.authoring/0.14`, new canonical capability `initialization.swarm_groups`; MCP server `3.24.0` adds `manage_showcase` so Professors publish their own Experiments to the Showcase through the MCP (owner request, 26 September); 4c heterogeneity syntax completed with the owner (26 September, `vlab.authoring/0.15`, MCP `3.25.0`): every group names its `dimension=`, `rest_of_group(...)` replaces `rest=True`, `within=` nests splits for exact joint counts, and groups give read-only traits (`set_trait`, Controller `NAME = trait(default)`, numbers or True/False); authoring guide `docs/HETEROGENEITY_GUIDE.md`; 4d alignment (26 September, `vlab.authoring/0.16`, MCP `3.26.0`): Metrics read True/False traits, editor autocomplete offers the Initialization intrinsics and `trait(...)`, the traits capability is renamed for display, memory versus traits wording in the scientific contract, and a recorded reference Experiment exercises dimensions, nesting and traits end to end; 5 statements in the environment field, done (`vlab.authoring/0.17`, MCP `3.27.0`): locals, `+=` and `if/elif/else` with a return on every path; single-return fields compile to the same simulator input as before). #577 is complete. **Experiment audit (26 September, owner request):** every Experiment was read against its paper and run on the simulator; the owner's six flagged Experiments got corrected revisions with Markdown explanation artifacts and Sergio's three toys were copied into the owner's space and corrected there, all through the MCP connector and grouped in the collection 'Audit fixes — 26 Sep 2026 (to check)'. The connector work exposed #592 (Metrics/Results tool compiled Metrics without the Controller state, references or Environment, and could not remove stale panels), fixed at MCP `3.28.0`; the same missing Metrics context in the Lab's open check blocked opening those Experiments, fixed in #594. #577 also replaces `set_agent_state` with experimenter-declared roles (D-022). MCP deployment is manual through the Supabase connector (`DEVELOPMENT_WORKFLOW.md`, Experiment MCP deployment).
