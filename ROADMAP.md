...
1. Repair #58's capability-request closed loop through #310 → #311 → #312 → #313 before implementing the queued scientific capabilities. #310/#311 establish durable blocked-Experiment preservation and comprehensive Student/Professor submission. #312 adds Professor-side resume and append-only whole-Experiment revalidation while preserving prior request lifecycle state. #313 is active: the first live Grok pass exposed fragmented repeated submission, so its bounded repair makes one scientific task converge on one blocked Experiment and reconciles matching nonterminal requests without lifecycle downgrade; the black-box Grok acceptance must then be rerun.
