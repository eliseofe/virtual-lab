...
impl LocalDensity {
    ...
    fn value(self) -> f64 {
        match self {
            Self::Low => LOW_LOCAL_DENSITY,
            Self::High => HIGH_LOCAL_DENSITY,
        }
    }
}
...
impl RadiusWorkload {
    fn label(self) -> &'static str {
        match self {
            Self::Single => "single",
            Self::ManyWide => "many-wide",
        }
    }
    fn radii(self) -> &'static [f64] {
        match self {
            Self::Single => SINGLE_RADII,
            Self::ManyWide => WIDE_RADII,
        }
    }
}
    ...
    for scale in [Scale::Low, Scale::High] {
        for local_density in [LocalDensity::Low, LocalDensity::High] {
            for workload in [RadiusWorkload::Single, RadiusWorkload::ManyWide] {
                let n = scale.n();
                let density = local_density.value();
                out.push(Scenario {
                    scale,
                    local_density,
                    workload,
...
fn assert_exact(s: &Scenario, state: &[AgentPhysicalState]) {
    ...
    current.rebuild(state, s.arena);
    multi.rebuild(state, s.arena);
    bvh.rebuild(state, s.arena);
    let mut expected = Vec::new();
    let mut actual = Vec::new();
    for &r in s.radii {
        for &agent in &probes {
            oracle.query(state, agent, r, s.arena, &mut expected);
            current.query(state, agent, r, s.arena, &mut actual);
            assert_eq!(
                actual,
                expected,
                "current mismatch scenario={} agent={} radius={}",
                s.id(),
                agent,
                r
            );
            multi.query(state, agent, r, s.arena, &mut actual);
            assert_eq!(
                actual,
                expected,
                "multi mismatch scenario={} agent={} radius={}",
                s.id(),
                agent,
                r
            );
            bvh.query(state, agent, r, s.arena, &mut actual);
            assert_eq!(
                actual,
                expected,
                "bvh mismatch scenario={} agent={} radius={}",
                s.id(),
                agent,
                r
            );
...
fn profile<I: BenchIndex>(mut idx: I, s: &Scenario, state: &[AgentPhysicalState]) -> ResultRow {
    let rebuild_ms = median_ms(|| idx.rebuild(black_box(state), s.arena));
    idx.rebuild(state, s.arena);
    ...
    let query_ms = median_ms(|| {
        ...
        for &r in s.radii {
            for agent in 0..state.len() {
                idx.query(state, agent, r, s.arena, &mut out);
                total += out.len();
            }
        }
        neighbours = total;
...
fn main() {
    println!("canonical_regime_matrix_version=1");
    println!("dimensions=scale(low:N100,high:N25000);local_density(low:0.25,high:16);radius(single:[1],many-wide:[0.1,1,10])");
    println!("fixture=uniform-periodic-lattice; density is used only to realize the requested local-neighbour-density regime");
    println!("timing=median-of-3; metric=rebuild+complete-query-phase");
    println!("result,scenario,scale,local_density,workload,n,density,arena,strategy,rebuild_ms,query_ms,combined_ms,entries,neighbours");

    for s in scenarios() {
        let state = uniform_grid(s.n, s.arena);
        ...
        let winner = rows
            .iter()
            .min_by(|a, b| a.combined_ms.total_cmp(&b.combined_ms))
            .unwrap();
        println!(
            "winner,{},{},{},{},{},{:.6}",
            s.id(),
