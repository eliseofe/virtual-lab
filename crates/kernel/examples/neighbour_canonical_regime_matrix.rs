...
impl Scenario {
    fn id(&self) -> String {
        format!("scale-{}-density-{}-{}", self.scale.label(), self.local_density.label(), self.workload.label())
    }
}
...
fn main() {
    ...
    for s in scenarios() {
        ...
        for row in &rows {
            println!(
                "result,{},{},{},{},{},{:.6},{:.6},{},{:.6},{:.6},{:.6},{},{}",
                s.id(), s.scale.label(), s.local_density.label(), s.workload.label(), s.n,
                s.density, s.arena, row.strategy, row.rebuild_ms, row.query_ms,
                row.combined_ms, row.entries, row.neighbours
            );
        }
        let winner = rows.iter().min_by(|a, b| a.combined_ms.total_cmp(&b.combined_ms)).unwrap();
        println!(
            "winner,{},{},{},{},{},{:.6}",
            s.id(), s.scale.label(), s.local_density.label(), s.workload.label(),
            winner.strategy, winner.combined_ms
        );
    }
}
