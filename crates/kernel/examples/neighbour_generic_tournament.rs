...
impl BenchIndex for MultiResolutionPeriodicGrid {
    ...
    fn entries(&self, _agents: usize) -> usize {
        MultiResolutionPeriodicGrid::index_entries(self)
    }
}
...
impl BenchIndex for AdaptivePeriodicBvh {
    ...
    fn entries(&self, _agents: usize) -> usize {
        AdaptivePeriodicBvh::index_entries(self)
    }
}
...
fn smoke_state(s: &SmokeScenario) -> Vec<AgentPhysicalState> {
    match s.distribution {
        ...
        SmokeDistribution::BoundaryBands {
...
fn build_panel(m: &Matrix) -> Vec<Scenario> {
    ...
    for &n in &a.agent_counts {
        for r in &a.radius_sets {
            add(&mut p, &mut seen, n, 1.0, r, Distribution::Uniform);
        }
    }
    for &d in &a.densities {
        for r in &a.radius_sets {
            add(&mut p, &mut seen, 5000, d, r, Distribution::Uniform);
        }
    }
    for dist in [Distribution::Clustered, Distribution::BoundaryBands] {
        for r in &a.radius_sets {
            add(&mut p, &mut seen, 5000, 1.0, r, dist);
        }
    }
    for dist in [Distribution::Clustered, Distribution::BoundaryBands] {
        for r in a.radius_sets.iter().skip(1) {
            add(&mut p, &mut seen, 25000, 1.0, r, dist);
        }
    }
    assert_eq!(p.len(), 34);
    p
    ...
    for s in &m.ci_smoke_scenarios {
        ...
        for &r in &s.radii {
            ...
            for agent in 0..state.len() {
                ...
                current.query(&state, agent, r, s.arena_size, &mut actual);
                assert_eq!(actual, expected);
                multi.query(&state, agent, r, s.arena_size, &mut actual);
                assert_eq!(actual, expected);
                bvh.query(&state, agent, r, s.arena_size, &mut actual);
                assert_eq!(actual, expected);
                reference.query_with_stats(&state, agent, r, s.arena_size, &mut actual);
                assert_eq!(actual, expected);
            }
...
fn assert_exact_probes(s: &Scenario, state: &[AgentPhysicalState]) {
    ...
    println!(
        "large_probe_exact,{},{}",
        s.id,
        probes.len() * s.radii.len()
    );
}

fn radii_text(r: &[f64]) -> String {
    r.iter()
        .map(|x| format!("{x:.6}"))
        .collect::<Vec<_>>()
        .join(";")
...
impl CurrentDiag {
    ...
    fn axis(&self, c: usize, span: usize) -> Vec<usize> {
        if span >= self.cells / 2 {
            return (0..self.cells).collect();
        }
        let n = self.cells as isize;
        (-(span as isize)..=(span as isize))
            .map(|o| (c as isize + o).rem_euclid(n) as usize)
            .collect()
    }
    fn stats(
        &self,
...
fn profile_diagnostics(s: &Scenario, state: &[AgentPhysicalState]) {
    ...
    let mut current = CurrentDiag::default();
    current.rebuild(state, s.arena_size);
    let mut multi = MultiResolutionPeriodicGrid::default();
    multi.rebuild(state, s.arena_size);
    let mut bvh = AdaptivePeriodicBvh::default();
    ...
    for &r in &s.radii {
        ...
        for &a in &probes {
            let (x, y) = current.stats(state, a, r, s.arena_size);
            cr += x;
            cc += y;
            let x = multi.query_with_stats(state, a, r, s.arena_size, &mut out);
            mr += x.visited_cells;
            mc += x.candidate_checks;
            level = x.level_index;
            let x = bvh.query_with_stats(state, a, r, s.arena_size, &mut out);
            br += x.visited_nodes;
            bc += x.candidate_checks;
            let x = reference.query_with_stats(state, a, r, s.arena_size, &mut out);
            rr += x.visited_cells;
            rc += x.candidate_checks;
        }
        println!(
            "diagnostic,current-periodic-grid,{},{:.6},{:.3},{:.3},{}",
            s.id,
            r,
            cr as f64 / q,
            cc as f64 / q,
        ...
        println!(
            "diagnostic,multi-resolution-periodic-grid,{},{:.6},{:.3},{:.3},{};level={}",
            s.id,
            r,
            mr as f64 / q,
            mc as f64 / q,
        ...
        println!(
            "diagnostic,adaptive-periodic-bvh,{},{:.6},{:.3},{:.3},{};depth={}",
            s.id,
            r,
            br as f64 / q,
            bc as f64 / q,
        ...
        println!(
            "diagnostic,radius-matched-reference-set,{},{:.6},{:.3},{:.3},{};cells={}",
            s.id,
            r,
            rr as f64 / q,
            rc as f64 / q,
...
fn control_anchor(s: &Scenario) -> bool {
    ...
    (s.agents == 1000 && s.density == 1.0 && s.distribution == Distribution::Uniform && moderate)
        ...
        || (s.agents == 5000
            && s.density == 1.0
            && s.distribution == Distribution::Clustered
            && moderate)
        || (s.agents == 5000
            && s.density == 1.0
            && s.distribution == Distribution::BoundaryBands
            && moderate)
}
...
fn main() {
    ...
    for (i, s) in panel.iter().enumerate() {
        ...
        let mut current = PeriodicGridNeighbourIndex::default();
        profile_generic(&mut current, s, &state);
        let mut multi = MultiResolutionPeriodicGrid::default();
        profile_generic(&mut multi, s, &state);
        let mut bvh = AdaptivePeriodicBvh::default();
        profile_generic(&mut bvh, s, &state);
        profile_reference(s, &state);
        profile_diagnostics(s, &state);

        if control_anchor(s) {
            let mut current = PeriodicGridNeighbourIndex::default();
            profile_control(&mut current, s, &state);
            let mut multi = MultiResolutionPeriodicGrid::default();
            profile_control(&mut multi, s, &state);
            let mut bvh = AdaptivePeriodicBvh::default();
            profile_control(&mut bvh, s, &state);
        }
