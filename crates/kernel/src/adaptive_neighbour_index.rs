...
fn build_node(
    ...
    *max_depth = (*max_depth).max(depth);
    ...
    indices[start..end].sort_by(|&a, &b| {
        let a_value = if split_x {
            state[a].position.x
        } else {
            state[a].position.y
        };
        let b_value = if split_x {
            state[b].position.x
        } else {
            state[b].position.y
        };
...
fn collect_candidates(
    ...
    if distance2_to_aabb(center_x, center_y, node) > pruning_radius2 {
        return;
    }
    if node.is_leaf() {
    ...
    collect_candidates(
        node.left.expect("internal left"),
        center_x,
        center_y,
        pruning_radius2,
        nodes,
    ...
    collect_candidates(
        node.right.expect("internal right"),
        center_x,
        center_y,
        pruning_radius2,
        nodes,
...
mod tests {
    ...
    fn boundary_bands(count: usize, arena_size: f64) -> Vec<AgentPhysicalState> {
        ...
        let per_side = count.div_ceil(4).max(1);
    ...
    fn assert_matches_brute_force(state: &[AgentPhysicalState], arena_size: f64, radii: &[f64]) {
        ...
        for &radius in radii {
            ...
            assert_eq!(
                bvh.structure_signature(),
                structure,
                "query radius changed BVH geometry"
            );
        }
    }
