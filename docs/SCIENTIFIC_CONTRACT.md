...
- **Heterogeneity is a composition, not an assignment (D-022, D-023).** The experimenter (Initialization) partitions the swarm into groups with exact sizes, given as a fraction of N, a count, or the rest; independent partitions are dealt independently. Groups are bound to bodies by a seeded uniform permutation or by explicit placement. Robot properties attach to groups: starting private state (`set_state`) and sensors (`equip`, with the range enforced by the simulator). No one sets state or equipment on an individual agent. The exact counts, and the seeded deal, keep compositions reproducible, with no run-to-run fluctuation in their sizes.
