-- #361: converge extension requests before Professor triage.
-- Keep the existing six-class taxonomy. One request is the durable decision unit;
-- later papers/Experiments attach evidence to that request when it plausibly covers
-- the same scientific/model need.

alter table public.capability_requests
  add column if not exists novelty_statement text;

alter table public.capability_requests
  drop constraint if exists capability_requests_novelty_statement_nonempty,
  add constraint capability_requests_novelty_statement_nonempty
    check (novelty_statement is null or length(btrim(novelty_statement)) > 0);

comment on column public.capability_requests.novelty_statement is
  'Durable comparison evidence for a genuinely new request: the materially distinct scientific/model need after active-request catalog review.';
...
create table public.active_extension_request_catalog (
  ...
  extension_domain text not null,
  extension_name text not null,
  extension_definition text not null,
  status text not null,
  updated_at timestamptz not null default now(),

  constraint active_extension_request_catalog_class
  ...
  constraint active_extension_request_catalog_status
    check (status in ('requested', 'approved', 'in_progress')),
  constraint active_extension_request_catalog_text_nonempty
    check (
      length(btrim(extension_key)) > 0
      and length(btrim(extension_domain)) > 0
      and length(btrim(extension_name)) > 0
      and length(btrim(extension_definition)) > 0
    )
);

comment on table public.active_extension_request_catalog is
  'Sanitized global catalog of active extension-request identity and scientific/model meaning for pre-triage research-AI reuse. It excludes requester, paper provenance, raw closure reasoning and Professor/developer notes.';

alter table public.active_extension_request_catalog enable row level security;
revoke all on table public.active_extension_request_catalog from public, anon, authenticated;
grant select on table public.active_extension_request_catalog to authenticated;

create policy "active_extension_request_catalog_authenticated_read"
on public.active_extension_request_catalog
for select
to authenticated
using (true);
...
begin
  ...
  if new.status in ('requested', 'approved', 'in_progress')
...
drop trigger if exists sync_active_extension_request_catalog
  on public.capability_requests;
create trigger sync_active_extension_request_catalog
...
select
  ...
  r.extension_domain,
  r.extension_name,
  r.extension_definition,
  r.status,
  r.updated_at
from public.capability_requests r
where r.status in ('requested', 'approved', 'in_progress')
  and r.request_class is not null
  and nullif(btrim(coalesce(r.extension_key, '')), '') is not null
  and nullif(btrim(coalesce(r.extension_domain, '')), '') is not null
  and nullif(btrim(coalesce(r.extension_name, '')), '') is not null
  and nullif(btrim(coalesce(r.extension_definition, '')), '') is not null
on conflict (request_id) do update
set request_class = excluded.request_class,
    extension_key = excluded.extension_key,
    extension_domain = excluded.extension_domain,
...
create table public.capability_request_evidence (
  ...
  closure_analysis_id uuid not null references public.capability_closure_analyses(id) on delete cascade,
  ...
  constraint capability_request_evidence_requirement_keys_array
    check (jsonb_typeof(requirement_keys) = 'array' and jsonb_array_length(requirement_keys) > 0)
);

create index capability_request_evidence_closure_idx
  on public.capability_request_evidence(closure_analysis_id);

comment on table public.capability_request_evidence is
  'Many-to-one evidence links from blocked-Experiment closure analyses to one durable extension request. Publication identity remains on the blocked Experiment/closure source.';

alter table public.capability_request_evidence enable row level security;
revoke all on table public.capability_request_evidence from public, anon, authenticated;
grant select, insert on table public.capability_request_evidence to authenticated;

create policy "capability_request_evidence_select_visible"
...
create policy "capability_request_evidence_insert_visible_closure"
on public.capability_request_evidence
for insert
to authenticated
with check (
  ...
  and exists (
    ...
    where a.id = capability_request_evidence.closure_analysis_id
      and (
...
-- Seed evidence for any typed request that predates this migration.
...
select
  r.id,
  r.closure_analysis_id,
  r.requester_id,
  r.requirement_keys,
  r.created_at
from public.capability_requests r
where r.closure_analysis_id is not null
  and jsonb_typeof(r.requirement_keys) = 'array'
  and jsonb_array_length(r.requirement_keys) > 0
on conflict (request_id, closure_analysis_id) do nothing;

-- Professor approval accepts the scientific need into the design queue.
-- Canonical semantic identity is resolved later in trusted developer generalization.
revoke insert on table public.canonical_capabilities from authenticated;
revoke insert on table public.capability_publication_provenance from authenticated;
revoke update (canonical_capability_id) on table public.capability_requests from authenticated;

drop policy if exists "canonical_capabilities_professor_insert"
