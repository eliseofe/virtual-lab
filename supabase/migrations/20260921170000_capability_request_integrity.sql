...
-- Reject ambiguous repeated reuse atomically in both structured entry points.
...
begin
  ...
  -- The evidence primary key is (request_id, closure_analysis_id). Reject
  -- repeated links before any writes instead of silently discarding keys or
  -- overwriting a generalization note with the last occurrence's relationship.
  if exists (
