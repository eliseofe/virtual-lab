-- #465: preserve immutable blocked-Experiment snapshots across capability closure.
-- Surgical changes only:
--   * automatic reuse requires the same origin revision;
--   * explicit stale snapshot reuse is rejected atomically;
--   * request rows inherit origin identity from the preserved blocked snapshot.
-- No capability/request scientific state, RLS, or candidate semantics change.
