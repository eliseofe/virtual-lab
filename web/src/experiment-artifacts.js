...
const GENERIC_TEXT_FORMATS = new Set(["python-vlab", "text/plain", "text/markdown", "markdown"]);
...
function coreArtifactsFromLegacy(experiment) {
  return EXPERIMENT_ARTIFACTS.map((descriptor) => ({
    ...
    content: experiment?.[descriptor.registryField] ?? "",
  }));
}

export function experimentArtifactArray(experiment) {
  const raw = Array.isArray(experiment?.artifacts)
    ? experiment.artifacts
    : coreArtifactsFromLegacy(experiment);
  const artifacts = raw.map(normalizeArtifact).sort((left, right) => left.order - right.order || left.id.localeCompare(right.id));
  const ids = new Set();
  ...
  for (const descriptor of EXPERIMENT_ARTIFACTS) {
    const artifact = artifacts.find((item) => item.id === descriptor.id);
...
export function sourceFieldsFromArtifactArray(artifacts) {
  ...
  return Object.fromEntries(
    EXPERIMENT_ARTIFACTS.map((descriptor) => [descriptor.registryField, byId.get(descriptor.id)?.content ?? ""]),
  );
}
...
export function artifactWritePayload(artifacts) {
  ...
  return {
    ...
    ...sourceFieldsFromArtifactArray(normalized),
  };
}
...
function renderGenericArtifact(root, container, artifact) {
  if (!GENERIC_TEXT_FORMATS.has(artifact.format)) {
    throw new Error(`Virtual Lab has no editor adapter for artifact '${artifact.id}' format '${artifact.format}'.`);
  }
  if (!container) {
    throw new Error(`Virtual Lab artifact UI mismatch: no container for additional artifact '${artifact.id}'.`);
  }
  ...
  kicker.textContent = artifact.type.toUpperCase();
...
export function captureExperimentArtifactArray(root = document) {
  ...
  if (container) {
    for (const editor of container.querySelectorAll('[data-experiment-artifact-editor="true"]')) {
      artifacts.push({
        id: editor.dataset.experimentArtifactId,
        type: editor.dataset.experimentArtifactType,
        label: editor.dataset.experimentArtifactLabel,
        format: editor.dataset.experimentArtifactFormat,
        order: Number(editor.dataset.experimentArtifactOrder),
        content: editor.value,
      });
    }
  }
...
export function captureExperimentArtifacts(root = document) {
  return artifactWritePayload(captureExperimentArtifactArray(root));
}
...
export function applyExperimentArtifacts(experiment, root = document) {
  ...
  for (const artifact of artifacts) {
    ...
    if (descriptor) {
      ...
      renderGenericArtifact(root, container, artifact);
    }
  }
}
...
export function experimentArtifactsEqual(left, right) {
  ...
  let rightArtifacts;
  try {
    leftArtifacts = experimentArtifactArray(left);
    rightArtifacts = experimentArtifactArray(right);
  } catch {
    return false;
  }
  ...
  return leftArtifacts.every((artifact, index) => {
    ...
    return artifact.id === other.id
      && artifact.type === other.type
      && artifact.label === other.label
      && artifact.format === other.format
      && artifact.order === other.order
