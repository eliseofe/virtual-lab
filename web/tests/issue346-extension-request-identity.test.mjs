...
test("#346 keeps requests separate from canonical capability identity", () => {
  assert.match(migration, /canonical_capability_id uuid[sS]*references public\.canonical_capabilities\(id\)/i);
  ...
  assert.match(migration, /implementation_state,[sS]*'not_implemented'/i);
...
test("#346 bumps the request/MCP interface without changing authoring language version", () => {
  ...
  assert.match(tools, /MCP_INTERFACE_VERSION = '12'/);
