...
test("#367 current documents contain one canonical-only surface and exact nine-tool contract", () => {
  ...
  assert.match(mcpDoc, /MCP server: `3\.12\.0`/);
  assert.match(mcpDoc, /interface: `16`/);
...
test("#367 versions the breaking connector cutover explicitly", () => {
  assert.match(metrics, /MCP_SERVER_VERSION = '3\.12\.0'/);
  assert.match(metrics, /MCP_INTERFACE_VERSION = '16'/);
